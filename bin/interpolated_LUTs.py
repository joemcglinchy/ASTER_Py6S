...
class Interpolated_LUTs_Gen:
  """
  The Interpolated_LUTs class handles loading, downloading and interpolating
  of LUTs (look up tables) used by the 6S emulator.
  """
  
  def __init__(self, mission, iLUT_directory):
    ...
    self.iLUTs_dir = iLUT_directory
    if not os.path.isdir(self.iLUTs_dir):
        print('iLUT directory {} does not exist. please try again\n'.format(self.iLUTs_dir))
    ...
    if filepaths:
      ...
      try:
        for f in filepaths:
          ...
          if self.mission == 'ASTER':
              bandName = self.ASTER_bandNames[bandName]
